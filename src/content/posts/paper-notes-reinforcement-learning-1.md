...
updated: 2026-09-11
description: 梳理 MDP、POMDP、Monte Carlo、Temporal Difference、Q-learning 与 Policy Gradient，重点说明问题设定、更新目标和收敛条件。
...
一条主线：**当环境模型（转移与奖励）未知时，如何仅凭采样经验估计价值并改进策略，以及这些过程在什么条件下收敛。** 在固定策略评估中，MC 直接使用完整回报作为样本目标，通常方差较高；TD 使用包含当前价值估计的自举目标，往往以一定偏差换取更及时、方差更低的更新。二者的张力贯穿整个 RL；Q-learning 与 Policy Gradient 则分别代表基于价值和直接优化策略的两条主要路线。
...
| [MC](#mc) | 蒙特卡洛方法 | 求解思路 | 用完整回合回报估值；固定策略下目标无自举偏差、方差通常较高 |
| [TD](#td) | 时序差分 | 求解思路 | **自举**：用当前估计构造目标，可在线、方差通常较低 |
...
MDP 用状态、动作、转移概率与奖励函数刻画满足 Markov 性的决策过程，但这并不要求智能体事先知道转移模型 $P$ 和奖励函数 $R$。智能体能直接观测 Markov 状态时通常按 MDP 建模；观测不足时使用 [POMDP](#pomdp)。当 $P,R$ 未知时，可以通过 [MC](#mc)、[TD](#td) 等 **model-free** 方法从交互样本中学习；状态空间过大时还需要函数逼近。参见 [Sutton & Barto, Ch. 3](https://www.incompleteideas.net/book/bookdraft2018mar21.pdf)。
...
:::note[一句话]
无模型价值估计的直接办法：**跑完整回合、平均观测到的回报**。在固定策略评估中，无需知道转移或奖励模型，完整回报是相应价值的无偏样本；代价是方差通常较高，而且本节的标准形式依赖会终止的回合式任务。
:::
...
- **首次访问 MC（first-visit）**：每回合只取 $s$ **首次**出现之后的回报平均。在固定策略、独立生成 episode，且状态被访问的条件下，这些回报可以作为 $v_\pi(s)$ 的无偏样本；样本平均随有效访问次数增加而收敛。若策略持续变化或轨迹相关，就不能直接把样本称为 i.i.d.。
- **每次访问 MC（every-visit）**：对 $s$ 的**每次**出现都计入。有限样本**有偏**（同回合内回报相关），但**一致**（访问数 → ∞ 时收敛）。

### 关键结果与性质

- **不使用自举目标，但方差通常较高**：在上述固定策略评估条件下，完整回报是无偏样本；整条轨迹累积的随机性会放大方差。
...
高方差和必须等回合结束，是回合式 MC 的两项主要限制。**TD 通过自举提供另一种取舍**：不必等到回合结束，目标方差通常更低，但会受当前价值估计误差影响。实际表现还取决于步长、函数逼近和数据分布。

### Takeaways

MC 给出「用完整经验回报估值」的基本范式。关键不是背诵“无偏、高方差”标签，而是说明条件：固定策略下完整回报可作为无偏样本，方差通常较高，标准更新要等到回合结束。
...
- **自举、可在线，方差通常较低**：目标依赖当前价值估计，因此有限样本误差中可能包含自举偏差；偏差与方差的实际大小还取决于步长、函数逼近和数据分布。
...
- **致命三要素（deadly triad）**：**函数逼近 + 自举 + 离策略**三者同时出现时可能发散——这是深度 RL 稳定性问题的重要来源。[DQN](/blog/posts/paper-notes-reinforcement-learning-2/#dqn) 的目标网络和经验回放在 Atari 实验中改善了稳定性，但不提供一般收敛保证。
- 标准步长条件：Robbins–Monro，$\sum_t\alpha_t=\infty,\ \sum_t\alpha_t^2<\infty$。

### 局限与延伸

自举目标依赖当前价值估计；在有限样本、函数逼近或参数持续更新时，误差可能传播，并在离策略训练中放大。TD 的优势是可在线更新且通常比完整回报具有更低方差，因此成为 Q-learning 等价值方法的基础。
...
表格 Q-learning 无法直接扩展到高维或连续状态。当多个动作价值估计含噪时，$\max$ 算子可能产生正偏差。用神经网络逼近 $Q$ 得到 [DQN](/blog/posts/paper-notes-reinforcement-learning-2/#dqn)：经验回放与目标网络改善训练稳定性，Double DQN 解耦动作选择和价值评估以缓解过估计；连续动作无法通过枚举求 $\max$，则需要 DDPG 等不同的策略参数化。
...
| MC | 否 | **是** | 否 | 固定策略下目标无偏 / 方差通常较高 | 皆可 | 价值（回报均值） |
| TD | 否 | 否 | **是** | 可能含自举偏差 / 方差通常较低 | 皆可 | 价值 |
...
2. **一个长期存在的取舍：偏差、方差与更新时效。** [MC](#mc) 等待完整回报，不使用自举目标；[TD](#td) 能在线自举更新，但目标受当前估计影响。TD(λ) 和 n-step return 在不同长度的实际回报与自举目标之间插值；是否使用 critic 还会同时改变样本效率和优化稳定性。

3. **致命三要素是暗线。** 「函数逼近 + 自举 + 离策略」同时出现会破坏收敛保证。本篇的收敛定理**只在表格 / 线性 on-policy 下成立**；第二篇的目标网络、双 Q、保守约束，很大程度都是在**工程上驯服**这个理论隐患。

4. **形式化决定了历史信息是否重要。** 标准 [MDP](#mdp) 的 Markov 状态已经包含预测未来所需的信息，策略可以只依赖当前状态；[POMDP](#pomdp) 的单步观测通常不足，常用历史、belief state 或记忆网络恢复相关信息。不过在部分特殊任务中，无记忆观测策略也可能已经足够。
