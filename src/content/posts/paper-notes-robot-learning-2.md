...
updated: 2026-09-11
description: 梳理 Physical Intelligence π 系列及相关方法，包括 π0、FAST、Knowledge Insulation、π0.5、RTC、Hi Robot、RECAP、RLT 与 π0.7。
image: '/paper-note/Robot_Learning_2/π0.7/teaser.png'
...
- **π0.7** — 可**操控**（steerable）的通才：多模态条件指定「怎么做」；官方报告展示了组合泛化与跨本体迁移的初步结果。
...
π0 提供了本篇后续工作的共同起点。相关研究分别处理动作表示与训练效率（FAST/KI）、跨环境泛化（π0.5）、推理延迟与动作连续性（RTC）、交互式语言条件（Hi Robot），以及利用经验数据改进策略（π*0.6）。这些工作针对的问题和评测设置不同，不构成单一指标上的线性升级。
...
- 本文只保留论文明确报告的总体结论：FAST 的 token 压缩显著提高了自回归动作训练效率，并在论文任务中取得与 flow-based π0 有竞争力的结果；逐任务数值请以原论文表格为准。
...
2. **用离散（FAST）动作 token 的 next-token 目标同时微调 VLM**：让骨干继续使用与语言预训练一致的离散序列目标；另有一个独立的 flow 动作专家输出连续动作。
...
- **作者主张**：论文将其结果概括为端到端学习系统在未参与训练的家庭环境中执行长时程灵巧操作；这一结论应按论文给定的家庭、任务和评测协议理解。
- **数据规模与消融**：官方报告显示，测试环境表现随训练环境数量增加而改善；在其规模曲线中，约 100 个训练环境时接近“在测试环境上训练”的参考线。移除 web 数据会降低新物体相关的 OOD 表现。这里只描述图表趋势，不引用无法与原图逐项对应的转述数字。
...
**π0.6** 是一个 5B VLM + 动作专家的通用 VLA 基座；**π*0.6**（pi-star-0.6）在其上应用 **RECAP**。RECAP 先做 advantage-conditioned offline RL 预训练，再用任务示范微调，并继续利用真机自主数据、专家纠正和奖励反馈进行专门化。π0.6 没有独立发布页，本节以 [π*0.6 官方报告](https://www.pi.website/blog/pistar06) 为准。
:::
...
- **核心机制——advantage 条件化**：先学习价值函数并据此估计动作相对基线的 advantage，再把 advantage 作为 VLA 的条件信号，使策略偏向高价值行为。价值的具体含义由任务奖励与报告定义，不能简单等同于“成功概率”。该表示可以统一示范、自主 rollout 和遥操作介入数据。
- **三阶段配方**：① 示范（人类遥操作）→ ② 教练/纠正（专家在自主执行中介入示范如何从错误里恢复）→ ③ 自主练习（机器人自采 rollout、用稀疏结果奖励，价值函数在线微调后再重新条件化策略）。
- **π*0.6 vs π0.6 vs π0.5**：π0.6 是一个 **5B VLM + 动作专家**；π0.5/π0.6 主要依赖监督模仿；π*0.6 先做 advantage-conditioned offline RL 预训练，再用任务示范和真机经验继续训练。这里的真机经验包括自主 episode、专家纠正与奖励反馈，不能概括为纯离线方法。
...
- **技术报告结果**：作者报告，在其选定的一组高难任务和系统配置中，RECAP 将任务吞吐提高到基线的两倍以上，并将失败率降低约一半；这些数字不应外推到其他任务或机器人平台。
- **结果口径**：官方报告分别用每小时成功完成次数和成功率评估装箱、叠衣与浓缩咖啡任务。本文保留报告可直接支持的结论：RECAP 在所有任务中提高吞吐，在部分困难任务上使吞吐和成功率超过两倍；不采用从图像近似读取的柱高作为精确数字。
...
π0.7 试图把 π 系列从任务专用策略推进到可由多模态条件控制的通才。官方报告称其观察到组合泛化的“初步迹象”，包括重组训练中已有技能以处理新任务；这些结果来自作者自报，尚不足以视为已经确立的类 LLM 泛化能力。[官方页面，2026-04-16](https://www.pi.website/blog/pi07)
...
把这些工作放在一起，可以比较 PI 围绕动作表示、泛化、实时执行和经验学习所做的不同设计选择：

1. **连续动作与离散动作各有取舍。** π0 用 flow matching 生成连续动作块，FAST 研究自回归动作 token 的训练效率，Knowledge Insulation 则把离散 token 的表征训练与连续动作专家结合起来。现有结果说明混合方案值得关注，但还不能据此认定它已经成为 VLA 的稳定终局。
...
4. **经验学习存在不同粒度。** π*0.6 用 RECAP 在整机策略层面结合 offline RL、示范和真机经验；RLT 从冻结 VLA 中抽取紧凑表征，再训练轻量 actor-critic，用于高精度任务的在线适配。π0.7 则通过语言、元数据、控制模态和视觉子目标组织异构数据，并把任务专家产生的数据纳入单个通才模型。三者的训练成本、交互需求和泛化口径不同，不宜写成必然的线性升级关系。

这些工作展示了几项可复用的设计选择：长程任务可使用分层语义子任务，动作表示会影响训练效率与语义知识保留，RL post-training 可利用自主经验改善专用任务，RTC 则针对动作块边界与推理延迟。它们是否能成为产品级通用方案，还需要跨平台、长期运行和安全性评测。
